...
function initDB() {
  ...
  // Telegram 桥接模式相关字段
  try { db.exec("ALTER TABLE moviepilot_channels ADD COLUMN channel_mode TEXT DEFAULT 'direct'"); } catch (_) {} // direct | telegram
  try { db.exec("ALTER TABLE moviepilot_channels ADD COLUMN telegram_bot_token TEXT DEFAULT ''"); } catch (_) {}
  try { db.exec("ALTER TABLE moviepilot_channels ADD COLUMN telegram_chat_id TEXT DEFAULT ''"); } catch (_) {}
  try { db.exec("ALTER TABLE moviepilot_channels ADD COLUMN telegram_proxy_enabled INTEGER DEFAULT 0"); } catch (_) {}
  try { db.exec("ALTER TABLE moviepilot_channels ADD COLUMN telegram_proxy_type TEXT DEFAULT 'http'"); } catch (_) {} // http | https | socks5
  try { db.exec("ALTER TABLE moviepilot_channels ADD COLUMN telegram_proxy_host TEXT DEFAULT ''"); } catch (_) {}
  try { db.exec("ALTER TABLE moviepilot_channels ADD COLUMN telegram_proxy_port INTEGER DEFAULT 0"); } catch (_) {}
