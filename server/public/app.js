...
(function () {
  ...
  function avatarHtml(name, url, size, hue) {
    if (url) {
      return `<img class="avatar" style="width:${size}px;height:${size}px" src="${esc(url)}" onerror="this.style.display='none'" />`;
    }
  ...
  async function uploadAndPublish(blob, kind, name) {
    try {
      ...
      const res = await fetch(`/api/topics/${encodeURIComponent(state.chat.topic)}/media?kind=${kind}`, {
        method: "POST",
